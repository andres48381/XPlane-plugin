...
class DataRef {
 ...
 public:
    ...
    //SIMLOC 231015 @andres
    // Retorna el valor del Identificador del DataRef
    std::string GetDataRefIdentifier();

    // Retorna el valor del DataRef
...
{
    m_pDataRef=XPLMFindDataRef(dataRefIdentifier.c_str());
}
...
//SIMLOC 231015 @andres
template <typename T>
std::string DataRef<T>::GetDataRefIdentifier()
{
        return m_dataRefIdentifier;
}

//SIMLOC 231015 @andres
template <typename T>
XPLMDataRef DataRef<T>::GetDataRef()
{
        return m_pDataRef;
}
